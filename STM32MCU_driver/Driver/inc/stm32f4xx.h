...
typedef struct
{
	...
	__vo uint32_t CMPCR;        /*!< Compension cell control register       								  Address offset: 0x20      */
 ...
 */
#define GPIOA_REG_RESET()               do{ (RCC->AHB1RSTR |= (1 << 0)); (RCC->AHB1RSTR &= ~(1 << 0)); }while(0)// First set the value with 1 in 0th position and then clear the value in 0th bit position
